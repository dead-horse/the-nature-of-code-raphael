...
(function (exports) {
  ...
  Boid.prototype.view = function (boids) {
    var sightDistance = 100;
    var periphery = Math.PI / 4;
    boids.forEach(function (b) {
      var comparison = PVector.sub(b.location, this.location);
      var d = PVector.dist(b.location, this.location);
      var diff = PVector.angleBetween(comparison, this.velocity);
  ...
  Boid.prototype.highlight = function () {
    if (!this.found) {
      this.found = true;
      this.line.attr({fill: '#f00'});
    }
  };

  Boid.prototype.stopHighlight = function () {
    if (this.found) {
      this.found = false;
      this.line.attr({fill: '#888'});
    }
  };

  Boid.prototype.select = function () {
    this.line.attr({fill: '#00f'});
  };
